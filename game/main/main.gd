...
# UI references
@onready var dev_tools_ui: DevToolsUI = $DevTools
...
func _on_regenerate_map_requested():
	if district_manager and pip_spawner:
		...
		# Set strategy to currently selected one from DevTools
		if dev_tools_ui:
			var current_strategy = dev_tools_ui.get_current_strategy()
			pip_spawner.set_spawn_strategy(current_strategy)
		
		# Regenerate pips with current strategy
		pip_spawner.respawn_pips()
