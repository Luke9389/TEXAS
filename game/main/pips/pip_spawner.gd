...
@export var pip_scene: PackedScene = preload("res://game/main/pips/entities/pip.tscn")
...
# Set the spawn strategy based on string name
func set_spawn_strategy(strategy_name: String):
	match strategy_name.to_lower():
		"random":
			spawn_strategy = RandomSpawnStrategy.new()
		"grid":
			spawn_strategy = GridSpawnStrategy.new()
		"clustered":
			spawn_strategy = ClusteredSpawnStrategy.new()
		_:
			push_warning("PipSpawner: Unknown strategy '" + strategy_name + "', using Random")
			spawn_strategy = RandomSpawnStrategy.new()
	
	print("PipSpawner: Set spawn strategy to ", strategy_name)
